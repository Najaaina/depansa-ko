...
export default function GoalDetailScreen() {
    ...
    Alert.alert("Archive Goal", "Are you sure you want to archive this goal?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          if (!user?.id || !id) return;
          try {
            await goalService.archive(user.id, id, goalId);
  ...
  const amountDiff = goal.amount - wallet.amount;
  const remainingAmount = amountDiff > 0 ? amountDiff : 0;
  const startDate = new Date(goal.startingDate).toLocaleString("en-US", {
    dateStyle: "long",
  });
  const endDate = new Date(goal.endingDate).toLocaleString("en-US", {
    dateStyle: "long",
  });
    ...
    >
      <View style={styles.header}>
        ...
        >
          <Ionicons
            name={
              (GOAL_TYPE_ICONS[goal.iconRef || "STAR"] ||
        ...
        <Text style={styles.walletName}>{goal.name}</Text>
        <Text
          style={styles.walletDescription}
        >{`Starting from ${startDate} to ${endDate}`}</Text>
      </View>

      <View style={styles.balanceCard}>
        <Text style={styles.balanceLabel}>Goal To Reach</Text>
        <Text style={styles.balanceAmount}>{formatAmount(goalAmount)}</Text>
        <View style={styles.statsRow}>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statLabel}>Remaining Amount</Text>
      ...
      <View style={styles.actions}>
        <Button
          title="Edit Goal"
          onPress={() => router.push(`/wallet/${id}/goal/${goalId}/edit`)}
          className="flex-1"
        />
        <Button
          title="Archive"
            ...
            <View style={styles.filterOptions}>
              {(["ALL", "IN", "OUT"] as const).map((t) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.filterOption,
                    filterType === t && styles.filterOptionActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      ...
                      filterType === t && styles.filterOptionTextActive,
                    ]}
                  >
        ...
        {transactions.length === 0 ? (
          ...
          transactions.map((transaction) => (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push(
                  `/wallet/${id}/edit-transaction?transactionId=${transaction.id}&walletId=${id}`,
                )
              }
            >
